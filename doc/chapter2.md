...
堆在虚拟机启动时创建。对象的堆存储由自动存储管理系统回收（即所谓的垃圾收集器），对象绝不会显式释放（译者注：即不会由程序员释放）。Java虚拟机不会假定任何类型的自动存储管理系统，该管理系统所用技术由实现者根据系统需求选择。堆可以是固定尺寸，也可以根据计算需求扩展，如果更大的堆变得没有必要，也会进行收缩。堆使用的内存不必是连续的。
...
局部变量数组和操作栈的尺寸在编译时确定，该值和帧的相关方法代码一起提供。因此，帧数据结构的尺寸不仅与Java虚拟机的实现有关，在方法调用的同时为这些结构分配的内存也有关系。

在拥有控制权的给定线程的任一时间点，只有正在执行方法的那一帧是活动的。该帧被称为当前帧，该方法被称为当前方法。定义该方法的类被称为当前类。局部变量和操作栈的操作通常与当前帧相关。

当帧的方法调用其他方法或者该方法完成了，该帧就不再是当前帧了。一个方法被调用，新的帧就被创建了，控制权转移到新的方法时，该帧就称为当前帧。当方法返回时，如果有返回值，则当前帧将结果传给前一帧。当前帧随后舍弃，前一帧变为当前帧。

记住，线程创建的帧是局部的，不能被其他线程引用。

#### 2.6.1 局部变量数组
每一帧含有一个称之为局部变量的变量数组。数组的长度在编译时决定，与帧相关的方法代码的类或接口的二进制表示中包含该值。

一个局部变量可以保存boolean、byte、char、short、int、float、reference、或returnAddress类型的值。一对局部变量可以保存long或double类型的值。

局部变量数组可以通过索引获取值。第一个局部变量的索引是0。只有0和局部变量长度-1之间的整数可以作为局部变量数组的索引（译者注：闭区间）。

long和double类型的值占用两个连续的局部变量。这样的值或许只能够通过较小的索引值获取。例如，一个double型的值保存在局部变量数组第n个索引，实际上占用了n和n+1两个局部变量；因此，位于n+1索引的局部变量不能被载入。该索引可以存储新值，但是这样做会破坏位于索引n的局部变量的内容。

Java虚拟机不要求n为偶数。直观上讲，局部变量数组中的long和double类型的值不必64位对齐。实现者可以自由决定合适方法表示这种保留两个局部变量的值。

Java虚拟机执行方法调用时，使用局部变量数组传递方法参数。在类中执行方法调用（译者注：静态方法），从索引0开始，接下来的局部变量都会作为参数传递。在类实例中执行方法调用，索引为0的局部变量总是用于传递方法调用所属的类实例的引用（例如Java编程语言中的this关键字）。从索引为1开始，接下来的连续变量以此作为参数传递。

#### 2.6.2 操作栈
每一帧都包括一个后进先出（LIFO）的栈，被称之为操作栈。帧的操作栈最大深度在编译器决定，该值和帧的相关方法代码一起提供。

在上下文明确的地方，有时候我们将简称当前帧的操作栈为操作栈。

当帧创建时，其操作栈是空的。Java虚拟机提供指令载入常量或从局部变量数组或者属性表（fields）载入值。其他Java虚拟机指令从操作栈获取操作指令并执行，然后将结果压入操作栈。操作栈也用于提供方法入参和获取方法结果。

举个例子，*iadd* 指令将两个int型值相加。它要求相加的两个数由之前的指令压入栈顶。两个int型值从操作栈弹出，然后相加，他们的和被压回操作栈。操作栈可能嵌套子计算过程，计算结果又会被总计算过程使用。

操作栈的每一项都可以保存Java虚拟机的任意类型值，包括long和double类型的值。

操作栈中的值必须以适用于其类型的方式进行操作。比如，不能压入两个int型值，接下来将他们作为long操作，或者压入两个float型值，然后使用 *iadd* 指令将他们相加。一小部分Java虚拟机指令（dup和swap指令）操作运行时数据区的原始数据，而不需要考虑他们各自的类型。这些指令被定义为不能修改或破坏单个的值。这类关于操作栈执行方式的限制通过class文件验证来强制实施（见4.10节）。

在任意时刻，操作栈有一个副深度，long型或double型占用两个单位深度，其他类型占用一个单位。

#### 2.6.3 动态链接
每一帧都包含当前方法类型的运行时常量池引用，以便支持方法代码的动态链接特性。class文件中方法代码指向将被调用的方法和通过符号引用获取的变量。动态链接将这些符号方法引用翻译为具体的方法引用，载入必要的类，以便解析还未确定的符号（as-yet-undefined symbols），通过这些变量运行时地址相关联的存储结构，将变量转换为合适的偏移量。

方法和变量的这种延迟绑定技术使得方法所调用的类即使有变化也不太可能破坏原有代码。

#### 2.6.4 方法调用正常完成
方法调用正常完成指的是，调用过程没有抛出异常，无论直接来自虚拟机的异常或者显式调用thorw语句抛出的异常。如果当前方法调用正常完成，可能会有返回值。如果有任何返回值，则调用方法执行的返回指令必须与该返回值类型相匹配。

这种情形中当前帧会恢复调用者的状态，包括它的局部变量数组和操作栈，调用者的程序计数器适当增加以跳过方法调用指令。然后调用方法的帧将返回值（如果有的话）压入操作栈，并继续正常执行。

#### 2.6.5 方法调用意外完成
一次方法调用意外完成指的是，方法中的Java虚拟机指令执行过程引起虚拟机抛出异常，并且异常没有被方法捕获。执行 *athrow* 操作也会引起异常被显式抛出，如果该异常没有被当前方法捕获，就会造成方法调用意外完成。方法调用意外完成永远都不会返回值给调用者。

### 2.7 对象的表示
Java虚拟机没有对对象内部结构有任何强制要求。

> 在一些Oracle的Java虚拟机实现里，对类实例的引用是一个执行句柄（handle）的指针。句柄本事是一对指针，其中一个指针指向一张表，该表包含对象的方法和表示对象类型的Class指针；另一个指针指向对象数据所分配的堆内存地址。

### 2.8 浮点数计算（略）

### 2.9 特殊方法
在Java虚拟机级别，Java编程语言实现的每个构造方法表现为一个实例初始化方法，它有个特殊的名字：<init>。这个名字是编译器提供的。因为<init>不是合法的标识符，它不能在Java编程语言程序中直接使用。实例初始化方法或许只能在Java虚拟机中通过 *invokespecial* 指令直接调用，并且只能在未初始化的类实例中调用。实例初始化方法从构造方法获得存取权限。

一个类或者单例最多有一个类或者接口初始化方法，并通过该方法初始化。类或实例的初始化方法
有一个特别的名字：<clinit>，没有任何参数，是void（参考4.3.3节）。

> class文件中其他名为<clinit>的方法没有任何影响。他们不是类或接口初始化方法。他们不能被任何Java虚拟机指令调用，永远都不会被Java虚拟机本身调用。

在class文件51.0及以上版本，方法必须额外有 ACC_STATIC 标记才能作为类或接口初始化方法。
